...
struct BadOrGood {
    ...
    BadOrGood(Bad b) { throw std::runtime_error(""); }

    BadOrGood(Good g) : Name(g.Name) {}
...
/*
    ...
    Хранимый тип имеет особенность: он выбрасывает исключение, когда конструктор
   будет вызван в третий раз

    Тест проверяет:
        1. Что конструктор выбросит исключение
        2. Для двух успешно скопированных объектов будет вызван деструктор
        3. Совпадает количество вызовов allocate и deallocate для некого типа
   Node (который будете вы использовать) *
        4. Совпадает количество аллоцированных и деаллоцированных элементов
   некого типа Node (который будете вы использовать) *

    * -- под Node имеется в виду какой-то ваш класс, которым вы описали ноду,
   имя класса не принципиально
*/
TEST_F(ExceptionSafetyTest, failesAtConstruct) {
    ...
    using unrolled_list_type =
        unrolled_list<SomeObj, 16, TestAllocator<SomeObj>>;
    ...
    ASSERT_EQ(TestAllocator<NodeTag>::AllocationCount,
              TestAllocator<NodeTag>::DeallocationCount);
    ASSERT_EQ(TestAllocator<NodeTag>::ElementsAllocated,
              TestAllocator<NodeTag>::ElementsDeallocated);
}
